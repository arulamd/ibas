...
FROM arAcctSubscriber 
    ...
    left join systemparameter systemparameterto on systemparameterto.divisioncode = arAcctSubscriber.migratedtodivisioncode and systemparameterto.companycode = arAcctSubscriber.migratedtocompanycode
        ...
        SELECT * FROM TEAMLEADERMASTER WHERE COMPANYCODE = 'COMCL' AND DIVISIONCODE = 'ISG'
        ORDER BY DIVISIONCODE  DESC
        
        SELECT * FROM IBAS.AGENTMASTER WHERE DIVISIONCODE = 'ISG' AND COMPANYCODE = 'COMCL'
        AND lower(AGENTNAME) LIKE lower('%ARISTOTLE%') ORDER BY AGENTCODE ASC
        
        SELECT * FROM (SELECT a.*, ROWNUM AS RNUM
        FROM (SELECT * FROM (SELECT * FROM IBAS.AGENTMASTER
        WHERE DIVISIONCODE = 'ISG' AND COMPANYCODE = 'COMCL'
        AND LOWER(AGENTNAME) LIKE LOWER('%ARISTOTLE%') ORDER BY AGENTCODE ASC) s) a  WHERE ROWNUM <= 20) WHERE RNUM > 10
        
        SELECT COUNT(*) FROM AGENTMASTER WHERE DIVISIONCODE ='ISG' AND COMPANYCODE  ='COMCL'
