...
for ll_row = 1 to ll_records
	...
	if not isNull(ls_newItemCode)  or not isNull(ls_newCAItemCode) then
		...
		----VALIADSI iuo_cpe.cancelPreAssignment() 
		setNull(acctNo)
		...
		---validasi fungsi f_scsControllerEnabled 
		long ll_count
		select count(*)
		  into :ll_count
		  from sysParamString
		...
		if f_scsControllerEnabled() THEN ( --IF RETURN TRUE)
			...
			--VALIDASI postSCSRequest(lsa_line_details, 'DE')
			string ls_xacctNo
		...
		--IF SERVICE TYPE = 'INET'
		--VALIDASI *NEW STB SERIAL NO iuo_cpe.setSerialNo(ls_newSerialNo)
		
		serialNo = trim(as_serialno) -- ls_newSerialNo
	  ...
	  if not iuo_cpe.setCASerialNo(ls_caSerialNo) then
		...
		--VALIDASI OLD ACCESS CARD  iuo_cpe.setCASerialNo(ls_caSerialNo)
		
		serialNo = trim(as_serialno) --ls_caSerialNo
		serialNo = UPPER(serialNo)
		...
		--VALIDASI NEW ACCESS CARDS iuo_cpe.setCASerialNo(ls_newCASerialNo)
		
		serialNo = trim(as_serialno) --ls_newCASerialNo
		serialNo = UPPER(serialNo)
		...
		if luo_subscriber.chargeTypeCode = 'PPS' then
			...
			--VALIDASI iuo_cpe.takePlaceInSTBPPCLoad(ls_newSerialNo) 
			update cpePrepaidLoad
				set serialNo = :as_newSerialNo --ls_newSerialNo
			 where serialNo = :serialNo
			 and divisionCode = :gs_divisionCode
			and companyCode = :gs_companyCode
			using SQLCA;
...
for ll_row = 1 to ll_records
	...
	if ls_selected = 'Y' and not IsNull(ls_newSerialNo) then
		...
		--VALIDASI iuo_cpe.takePlaceInJOAssignedDigiPick(ls_serialNo, ls_caSerialNo, ls_newSerialNo, ls_newCASerialNo, is_joNo)
		update joTranDtlAssignedDigiPick
		set    stbSerialNo = :as_newSerialNo, --ls_newSerialNo
				 caSerialNo  = :as_newCASerialNo --ls_newCASerialNo
		where  joNo        = :as_joNo
		and    stbserialNo    = :as_serialNo  --ls_serialNo
		and    caSerialNo  = :as_caSerialNo --ls_caSerialNo
		and    divisionCode = :gs_divisionCode
		and    companyCode  = :gs_companyCode
		using  SQLCA;
		if SQLCA.SQLCode <> 0 then
			lastSQLCode 		= string(SQLCA.SQLCode)
			lastSQLErrText 	= SQLCA.SQLErrText + &
									  '~r~nError Updating table [joTranDtlAssignedDigiPick]...'
			return false
		end if	
	...
	if f_scsControllerEnabled() then
		...
		--get VALUE ls_convertedSno = convertSerialNo(serialNo)
		int    ll_dgit
		long   ll_length, ll_loop
		string ls_serialNo, ls_curString
		
		ll_dgit = 1
		
		
		for ll_loop = 1 to 10
			ls_curString = mid(as_serialno, ll_loop,1)
			
			if ls_curString = 'A' then
				ls_curString = '0'
			elseif ls_curString = 'B' then 
				ls_curString = '1'
			elseif ls_curString = 'C' then
				ls_curString = '2'
			elseif ls_curString = 'D' then	
				ls_curString = '3'		
			elseif ls_curString = 'E' then			
				ls_curString = '4'		
			elseif ls_curString = 'F' then			
				ls_curString = '5'
			elseif ls_curString = 'G' then			
				ls_curString = '6'
			elseif ls_curString = 'H' then			
				ls_curString = '7'
			elseif ls_curString = 'J' then
				if ll_loop = 1 then
					ls_curString = '0'
				else
					ls_curString = '8'
				end if	
			elseif ls_curString = 'K' then
				ls_curString = '9'		
			elseif ls_curString = 'L' then
				ls_curString = 'A'		
			elseif ls_curString = 'M' then
				ls_curString = 'B'		
			elseif ls_curString = 'N' then
				ls_curString = 'C'		
			elseif ls_curString = 'P' then
				ls_curString = 'D'				
			elseif ls_curString = 'Q' then
				ls_curString = 'E'		
			elseif ls_curString = 'R' then
				ls_curString = 'F'				
			elseif ls_curString = 'T' then
				ls_curString = '1'
			end if	
			
			if ll_dgit = 2 then
				ll_dgit = 1
				ls_serialNo = ls_SerialNo + ls_curString + ";"
			else
				ls_serialNo = ls_SerialNo + ls_curString
				ll_dgit ++
			end if	
		...
		--GET VALUE generateBoxIDCommand(boxIdNo, ls_command)
		
		Long temp, temp1x

		temp = (((al_boxId - 1) / 262144) + 224) 
		...
		as_boxCommand = replace(as_boxCommand, 2, 1, Char(temp1x))
		...
		as_boxCommand = replace(as_boxCommand, 3, 1, Char(temp1x) + ';')
		
		temp = (( mod((al_boxId - 1), 262144) / 4096) + 128) 
		...
		as_boxCommand = replace(as_boxCommand, 5, 1, Char(temp1x))
		...
		as_boxCommand = replace(as_boxCommand, 6, 1, Char(temp1x) + ';')
		 
		temp = ((mod(mod((al_boxId - 1), 262144), 4096) / 64) + 64) 
		...
		as_boxCommand = replace(as_boxCommand, 8, 1, Char(temp1x))
		...
		as_boxCommand = replace(as_boxCommand, 9, 1, Char(temp1x) + ';')
		
		 
		temp = mod(mod(mod((al_boxId - 1), 262144), 4096), 64) 
		...
		as_boxCommand = replace(as_boxCommand, 11, 1, Char(temp1x))
		...
		as_boxCommand = replace(as_boxCommand, 12, 1, Char(temp1x))
		------------------------------------------------
