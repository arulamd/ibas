--WHEN OPEN WINDOW OBJECT , CHECK AUTORIZtion pb_assignstbtoejo

select count(*) into :ll_count
from userrightsmaster
where usercode = :gs_usercode 
		and rightscode = 'JOB01'
		and companycode = :gs_companycode
		and divisioncode = :gs_divisioncode
using SQLCA;
if SQLCA.SQLCode < 0 then
	guo_func.msgBox('Error!',SQLCA.SQLErrText)
	return
end if

if ll_count > 0 then
	uo_options.pb_assignstbtoejo.enabled = true
end if
...
left join joremarksmaster on joremarksmaster.remarkscode = jotranhdr.jostatusremarkscode
 WHERE ( ( :as_alltrantype = 'Y' ) or joTranHdr.tranTypeCode in (:as_trantypecode) )   
  ...
  and ( joTranHdr.divisionCode = :as_division ) and ( joTranHdr.companyCode = :as_company ) 
 and (arPackageMaster.isDigital = :as_sigNalType or :as_allSigNalType = 'Y')